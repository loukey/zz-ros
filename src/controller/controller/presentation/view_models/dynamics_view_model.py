...
class DynamicsViewModel(BaseViewModel):
    ...
    Attributes:
        recording_state_changed (pyqtSignal): 记录状态变化信号。
        record_list_updated (pyqtSignal): 记录列表更新信号。
        command_hub (CommandHubService): 命令中心服务。
        robot_state (RobotStateDomainService): 机械臂状态服务。
        dynamic_service (DynamicDomainService): 动力学服务。
        teach_record_service (TeachRecordDomainService): 示教记录服务。
        record_repository (RecordRepository): 记录仓储。
    """
    ...
    def _on_torque_compensation_requested(self, joint_angles: list):
        ...
        Args:
            joint_angles (list): 当前关节角度（已减去初始偏置）。
        """
        try:
            # 1. 计算重力补偿
