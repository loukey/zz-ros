...
def sample_linear_se3(p0, q0, p1, q1, ds=0.02, include_end=True):
    """
    直线等距采样（步长 ds，单位 m），姿态用 SLERP。
    p0,p1: (3,) 位置（米）
    q0,q1: (4,) 四元数 [w, x, y, z] 或 [x, y, z, w] 都行（下面自动兼容）
    返回: (poses_xyz[N,3], poses_quat[N,4])
    """
    ...
    if L < 1e-9:
        ...
        n_seg = max(1, int(np.ceil(L / float(ds))))
        t = np.linspace(0.0, 1.0, n_seg + 1 if include_end else n_seg)

    # 位置线性插值
    poses_xyz = (p0[None, :] + (p1 - p0)[None, :] * t[:, None])

    # 姿态 SLERP
    poses_quat = np.vstack([_q_slerp(q0, q1, ti) for ti in t])

    return poses_xyz, poses_quat

def sample_along_base_Z(p0, q0, dz, ds=0.02, include_end=True):
    """
    沿“基坐标系 Z 轴”做直线等距采样（位置步长 ds，单位 m），姿态保持不变。
    参数：
      p0 : (3,) 起点位置 [x,y,z]（米）
      q0 : (4,) 起点四元数 [w,x,y,z]
      dz : 末端相对位移（米），>0 向 +Z（上），<0 向 -Z（下）
      ds : 采样步长（米），默认 0.02
      include_end : 是否包含终点采样点
    返回：
      poses_xyz (N,3), poses_quat (N,4)
    """
    p0 = np.asarray(p0, dtype=float).reshape(3)
    q0 = _q_normalize(q0)
    ...
    # 直线方向：基坐标 Z 轴
    d = np.array([0.0, 0.0, dz], dtype=float)

    # 位置线性采样；姿态保持不变
    poses_xyz = p0[None, :] + t[:, None] * d[None, :]
    poses_quat = np.repeat(q0[None, :], len(t), axis=0)
